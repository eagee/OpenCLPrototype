...
static void md5Round(uint* internalState, const uint* key) {
    uint a, b, c, d;
    a = internalState[0];
    b = internalState[1];
    c = internalState[2];
...
void md5(const char* restrict message, uint lengthInBytes, uint* restrict outputBuffer) 
{
    uint i;
    uint bytesLeft;
    char key[64];
    ...
    {
        md5Round(outputBuffer, (const uint*) message);
    }
    ...
    else 
    {
        ...
        md5Round(outputBuffer, (uint*) key);
 ...
 __kernel void createChecksum(__global __read_only unsigned char *fileData, __global __read_only size_t *fileLength, __global md5Hash *resultsBuffer)
 {
     int checksum = 0;
    ...
    uint startingOffset = (get_global_id(0) * fileLength[0]);
